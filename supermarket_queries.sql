...
select branch_name, ROUND(SUM(total), 2) as TotalPerBranch 
from invoice
...
SELECT 
    ...
    p.product_line,
    c.gender,
    SUM(id.quantity * id.unit_price) AS total_spent
FROM 
    invoice_details
...
GROUP BY b.branch_name, p.product_line, c.gender
ORDER BY b.branch_name, p.product_line, c.gender;
